...
run_case() {
    ...
    local out="$BUILD/tb_flash_attn_top_e2e_lowprecision_${name}.vvp"
    local hex="$BUILD/tb_flash_attn_top_e2e_lowprecision_${name}_o.hex"
    ...
    iverilog -g2012 -Wall \
        ...
        -P tb_flash_attn_top_e2e_smoke.DATA_W=8 \
        -P tb_flash_attn_top_e2e_smoke.FRAC_W=4 \
        -P tb_flash_attn_top_e2e_smoke.SOFTMAX_FRAC=8 \
...
run_fp8_case() {
    ...
    local out="$BUILD/tb_flash_attn_top_e2e_fp8_${name}.vvp"
    local hex="$BUILD/tb_flash_attn_top_e2e_fp8_${name}_o.hex"
    ...
    iverilog -g2012 -Wall \
        ...
        -P tb_flash_attn_top_e2e_smoke.FP8_E4M3_MODE=1 \
        -P tb_flash_attn_top_e2e_smoke.VALID_LEN="$s_len" \
        -P tb_flash_attn_top_e2e_smoke.SCALE_Q8_8="$scale" \
        -P tb_flash_attn_top_e2e_smoke.CHECK_BITEXACT=0 \
        -P tb_flash_attn_top_e2e_smoke.TIMEOUT_CYCLES="$timeout_cycles" \
    ...
    run_vvp "$out" "+OUT_HEX=$hex"
    check_fp8_output "$hex" "$s_len" "$d_model" "$bk" "$scale"
}

run_case "s8_d8" 8 8 4 4 91 200000 80000
run_case "s32_d16" 32 16 8 8 64 800000 120000

run_fp8_case "s8_d8" 8 8 4 4 91 200000 80000
run_fp8_case "s32_d16" 32 16 8 8 64 800000 120000

if [[ "${RUN_FULL:-0}" == "1" ]]; then
    run_case "s256_d64" 256 64 16 16 32 2500000 300000
    run_fp8_case "s256_d64" 256 64 16 16 32 2500000 300000
else
    echo "Skipping full-size low-precision smoke; rerun with RUN_FULL=1 for S=256,D=64."
fi

echo "Bonus low-precision INT8/Q4.4 and FP8/E4M3 smoke checks passed."
