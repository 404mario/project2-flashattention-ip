...
分支：`baseline-v2-synthopt`（v2 流式核；8ns 参照为 `8nsclean-baseline`，233,312-cycle / 8ns-clean）
...
每阶段都用现有 SV 自检 testbench（`tb_flash_attn_top_e2e_smoke.sv`,iverilog,不需 numpy）验功能 + 量 cycles；保底版 `8nsclean-baseline` 永不动。
...
|---|---:|---:|---:|---:|---|
| baseline (`8nsclean-baseline`) | 233,312 | — | 0.000097 | 0.054688 | PASS |
