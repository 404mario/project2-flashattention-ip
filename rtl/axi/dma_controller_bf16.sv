...
module dma_controller_bf16 #(
    ...
    localparam int WORDS_PER_BEAT = AXI_BYTES / 2;
    localparam int ROW_BYTES      = D_MODEL * 2;
    ...
    logic [15:0] o_bf16_buf [0:D_MODEL-1];
        ...
        if (state_q == S_O_WR_SEND) begin
            for (comb_word = 0; comb_word < WORDS_PER_BEAT; comb_word = comb_word + 1) begin
                ...
                if (comb_index < D_MODEL) begin
                    wr_data[(comb_word * 16) +: 16] = o_bf16_buf[comb_index];
                end
            end
        end
                    ...
                    S_IDLE: begin
                        ...
                        if (q_req_valid) begin
                            ...
                            for (seq_col = 0; seq_col < D_MODEL; seq_col = seq_col + 1) begin
                                ...
                                o_bf16_buf[seq_col] <=
                                    bf16_pkg::q8_8_to_bf16(o_data_flat[seq_col * CORE_DATA_W +: CORE_DATA_W]);
                            end
                            beat_idx_q <= '0;
                            state_q    <= S_O_WR_REQ;
                        end
                        ...
                        if (rd_data_valid && rd_data_ready) begin
                            for (seq_word = 0; seq_word < WORDS_PER_BEAT; seq_word = seq_word + 1) begin
                                ...
                                if (seq_col < D_MODEL) begin
                                    q_buf[seq_col] <= bf16_pkg::bf16_to_q8_8(rd_data[(seq_word * 16) +: 16]);
                                end
                            end
                            ...
                            for (seq_word = 0; seq_word < WORDS_PER_BEAT; seq_word = seq_word + 1) begin
                                ...
                                if (seq_col < D_MODEL) begin
                                    k_buf[tile_row_idx_q][seq_col] <=
                                        bf16_pkg::bf16_to_q8_8(rd_data[(seq_word * 16) +: 16]);
                                end
                            end
                            if (rd_last || (beat_idx_q == ROW_BEATS - 1)) begin
