...
- **问题 B（bonus-v2 既有，非本次引入，未修）**：checker 的「RTL vs 定点镜像」**1-LSB 硬门**（`check_top_e2e_output.py:600-602`，无容差旋钮）对 lossy/中点舍入 case 报错（MaxE 0.0039 ≪ FP32 容差 0.10）；以及 dropout 全丢行边角（**经核 `normalizer.sv:255` 已有 `s1_zero_q<=(denom==0)` → `rounded_shift_abs(zero)` 返回 0 的 guard，先前 agent"无 guard"的说法不准确**；该边角属 checker 镜像/语义问题）。**两者在原 bonus-v2 树上以完全相同的数值复现**，超出"换骨架"范围，留作 owner 决定（如给镜像门加容差）。
